...
class TrigQuizApp {
    constructor() {
        ...
        this.dom = {
            ...
            referenceCurrentAngle: document.getElementById('reference-current-angle'),
            referenceSinValue: document.getElementById('reference-sin-value'),
            referenceCosValue: document.getElementById('reference-cos-value'),
            referenceTanValue: document.getElementById('reference-tan-value'),
    ...
    init() {
        ...
        // Restore BGM track choice
        const savedBgmTrack = localStorage.getItem('trig-quiz-bgm-track');
        if (savedBgmTrack && this.dom.headerBgmSelect) {
            this.dom.headerBgmSelect.value = savedBgmTrack;
            this.bgmTrack = savedBgmTrack;
        }
    }
    ...
    updateReferenceGuide(deg) {
        ...
        if (this.referenceGuideVisualizer) {
            this.referenceGuideVisualizer.update(deg, 'sin');
            this.referenceGuideVisualizer.setInteractive(true);
        }
        ...
        if (this.dom.referenceExplanation) {
            this.dom.referenceExplanation.innerHTML =
                `${deg}° の値： sin = ${window.formatValueText(data.sin.valueId, this.useRationalized)}、 ` +
                `cos = ${window.formatValueText(data.cos.valueId, this.useRationalized)}、 ` +
                `tan = ${window.formatValueText(data.tan.valueId, this.useRationalized)}。`;
        }
    }
    ...
    startQuiz() {
        ...
        if (this.mode === '20-challenge') {
            ...
            this.dom.quizProgressBar.style.width = '0%';
            ...
            this.dom.quizModeBadge.textContent = '⏱️ 3分チャレンジ';
            this.dom.timerContainer.style.display = this.timeLimitSetting > 0 ? 'block' : 'none';
            this.dom.livesContainer.style.display = 'none';
            this.timePerQuestion = this.timeLimitSetting > 0 ? this.timeLimitSetting : 10;
            this.globalTimeLeft = 180;
            ...
            if (this.dom.globalTimerText) {
                this.dom.globalTimerText.style.display = 'block';
                this.dom.globalTimerText.textContent = '残り 3:00';
            }
    ...
    nextQuestion() {
        ...
        if (this.isReviewSession || this.mode === '20-challenge') {
            ...
            this.dom.quizProgressText.textContent = `第 ${this.questionIndex} 問 ／ 正解 ${correctSoFar} 問`;
        } else {
    ...
    updateGlobalChallengeDisplay() {
        ...
        const pct = Math.max(0, (this.globalTimeLeft / 180) * 100);
    ...
    tryFinalizeAnswer() {
        ...
        if (this.mode === '20-challenge' && this.questionIndex >= this.totalQuestions) {
            ...
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 1400);
        } else if (this.mode === '3min-challenge') {
            this.dom.btnNextQuestion.textContent = '次の問題へ ➔';
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 1200);
        } else {
            this.dom.btnNextQuestion.textContent = '次の問題へ ➔';
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 1400);
        }
    ...
    handleTimeout() {
        ...
        if (this.isReviewSession && this.questionIndex >= this.totalQuestions) {
            this.dom.btnNextQuestion.textContent = '復習結果を見る ➔';
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 2200);
        } else if (this.mode === 'endless' && this.lives <= 0) {
            this.dom.btnNextQuestion.textContent = '結果を見る ➔';
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 1800);
        } else if (this.mode === '20-challenge' && this.questionIndex >= this.totalQuestions) {
            this.dom.btnNextQuestion.textContent = '結果発表へ ➔';
            this.autoAdvanceTimer = setTimeout(() => this.nextQuestion(), 1400);
        } else {
    ...
    finishQuiz() {
        ...
        if (scoreLabel) scoreLabel.textContent = this.mode === '3min-challenge' ? '3分間の正解数' : 'スコア';
        ...
        const angleErrors = this.history.filter(h => !h.circleCorrect).length;
        const valueErrors = this.history.filter(h => !h.valueCorrect).length;
        const weak = stats.slice().sort((a,b) => a.pct - b.pct)[0];
                ...
                <div class="analysis-bars">
                    ${stats.map(s => `
                        <div class="analysis-row">
                            <span class="analysis-label func-${s.func}">${funcLabels[s.func]}</span>
                            <div class="analysis-track"><span style="width:${s.pct}%"></span></div>
                            <strong>${s.pct}%</strong>
                        </div>
                    `).join('')}
                </div>
                <div class="analysis-advice">
                    ${weak && weak.pct < 100 ? `重点復習：<strong>${weak.func}</strong> が ${weak.pct}% です。` : '三角比ごとの正答率は良好です。'}
                    ${angleErrors > valueErrors ? ' 単位円上の「角度の位置」の確認を優先しましょう。' :
                      valueErrors > angleErrors ? ' 三角比の「値」の暗記・対応関係を優先しましょう。' :
                      angleErrors > 0 ? ' 角度の位置と値を同じくらい復習しましょう。' : ''}
                </div>
            `;
        }
    ...
    savePersonalBest() {
        ...
        if (this.mode === '20-challenge') {
            ...
            const key = 'trig-quiz-best-3min-challenge';
    ...
    updatePersonalBestDisplay() {
        ...
        const best3min = Number(localStorage.getItem('trig-quiz-best-3min-challenge'));
    ...
    showReferenceModal() {
        if (this.dom.quizScreen.classList.contains('active')) {
            this.lastScreenBeforeReference = 'quiz';
        } else if (this.dom.resultScreen.classList.contains('active')) {
            this.lastScreenBeforeReference = 'result';
        } else {
            this.lastScreenBeforeReference = 'start';
        }
    ...
    hideReferenceModal() {
        this.showScreen(this.lastScreenBeforeReference || 'start');
    }
}
