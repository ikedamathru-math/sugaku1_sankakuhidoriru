...
class UnitCircleVisualizer {
    ...
    renderBase() {
        this.container.innerHTML = `
            <svg viewBox="${(this.container && (this.container.id === 'reference-guide-circle' || this.container.id === 'quiz-unit-circle')) ? '-225 -225 450 450' : '-175 -155 350 178'}" class="unit-circle-svg" aria-label="単位円の図解">
                ...
                <line x1="${this.R}" y1="${this.container && this.container.id === 'reference-guide-circle' ? -218 : (this.container && this.container.id === 'quiz-unit-circle' && window.matchMedia && window.matchMedia('(max-width: 767px)').matches ? -205 : -218)}" x2="${this.R}" y2="${this.container && this.container.id === 'reference-guide-circle' ? 218 : (this.container && this.container.id === 'quiz-unit-circle' && window.matchMedia && window.matchMedia('(max-width: 767px)').matches ? 205 : 218)}" class="tan-guide-line" />
    ...
    update(deg, func = 'all', userSelectedDeg = null) {
        ...
        if (deg === 90) {
            ...
            const tanLimit = isPhoneQuizTan ? 205 : (isExpandedCircle ? 218 : 145);
